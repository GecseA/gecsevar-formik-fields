...
export function GVSelect({
  ...
  return isMultiSelect ? (
    <FormControl sx={{ margin: '5px', width: 300 }} error={errorMessage == undefined ? false : errorMessage?.length !== 0}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
            {selected.map((val) => (
              <Chip
                ...
                size={'small'}
                onDelete={() => {
                  const temp: Array<string> = getIn(values, field.name).filter((item: string) => {
                    return item !== val
                  })
    ...
    <TextField
      style={{ minWidth: '200px', maxWidth: '300px', margin: '5px' }}
      {...field}
      {...props}
      size={'small'}
      select
